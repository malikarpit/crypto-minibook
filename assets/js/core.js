...
/* ── EVENT DELEGATION (Copy, Bookmarks, Recall Blocks) ───────────────── */
function initDelegatedEvents() {
  document.body.addEventListener('click', e => {
    ...
    // 4. Active Recall — toggle answer visibility on question click
    const rq = e.target.closest('.recall-question');
    if (rq) {
      const ans = rq.nextElementSibling;
      if (ans && ans.classList.contains('recall-answer')) {
        ans.classList.toggle('visible');
        rq.style.borderLeftColor = ans.classList.contains('visible') ? 'var(--adv-c)' : 'rgba(139,92,246,0.4)';
      }
    }

    // 5. Reveal buttons in recall blocks
    const revealBtn = e.target.closest('.reveal-btn');
    if (revealBtn) {
      const block = revealBtn.closest('.recall-block');
      if (block) {
        block.querySelectorAll('.recall-answer').forEach(a => a.classList.add('visible'));
        revealBtn.textContent = '✅ All revealed — click questions to toggle';
